...
public class UIPanelInspector : UIRectEditor
{
    ...
    protected override void OnDisable()
    {
        ...
        {
            tweeners[ i ].m_PingPongOneCount = 0;
            tweeners[ i ].m_PreTime = 0f;
            tweeners[ i ].isPlay = false;
            tweeners[ i ].isStart = false;
        }
    }
    ...
    {
        Selection.activeGameObject = mPanel.gameObject;
        Handles.BeginGUI();
        Rect windowRect = new Rect( Screen.width - 210, 25, 200, 85 );
        GUI.Window( 2, windowRect, DrawNodeWindow, new GUIContent( "Tween Components Preview" ) );
        Handles.EndGUI();
        ...
        {
            UIWidgetInspector.DrawAnchorHandle( ref mPanel.leftAnchor, mPanel.cachedTransform, handles, 0, id );
            UIWidgetInspector.DrawAnchorHandle( ref mPanel.topAnchor, mPanel.cachedTransform, handles, 1, id );
            UIWidgetInspector.DrawAnchorHandle( ref mPanel.rightAnchor, mPanel.cachedTransform, handles, 2, id );
            UIWidgetInspector.DrawAnchorHandle( ref mPanel.bottomAnchor, mPanel.cachedTransform, handles, 3, id );
        }
    ...
    private void DrawNodeWindow( int id )
    {
        ...
        if ( GUILayout.Button( !isPlay ? "Play" : "Pause", "ButtonLeft" ) )
        {
            isPlay = !isPlay;
            for ( int i = 0; i < tweeners.Count; i++ )
            {
                if ( showGroup && tweeners[ i ].tweenGroup != group )
                    continue;
                tweeners[ i ].OnPlayOrPause( isPlay );
            }
        ...
        for ( int i = 0; i < tweeners.Count; i++ )
        {
            if ( !list.Contains( tweeners[ i ].tweenGroup ) )
                list.Add( tweeners[ i ].tweenGroup );
        }
        list.Sort();
        ...
        if ( !list.Contains( group ) && list.Count > 0 )
            group = list[ 0 ];

        GUILayout.BeginHorizontal();
        showGroup = EditorGUILayout.ToggleLeft( "Specify Tween Group", showGroup, GUILayout.Width( 140f ) );
        if ( showGroup )
            group = EditorGUILayout.IntPopup( group, strArr, arr );
        GUILayout.EndHorizontal();
    ...
    protected override bool ShouldDrawProperties()
    {
        ...
        for ( var _it = UIPanel.list.First; _it != null; _it = _it.Next )
        {
            UIPanel p = _it.Value;
    ...
    protected override void DrawFinalProperties()
    {
        ...
        if ( NGUIEditorTools.DrawHeader( "Tween Components" ) )
        {
            NGUIEditorTools.BeginContents();

            tweeners = mPanel.gameObject.GetComponentsInChildren<UITweener>().ToList();

            if ( tweeners.Count == 0 )
            {
                EditorGUILayout.HelpBox( "There is no Tween Component in children.", MessageType.Info );
            }

            for ( int i = 0; i < tweeners.Count; i++ )
            {
                DrawTweenerProperties( tweeners[ i ] );
                if ( i < tweeners.Count - 1 )
                {
                    GUILayout.Space( 6f );
                }
            }
    ...
    private void TweenUpdate( int i )
    {
        UITweener tweener = tweeners[ i ];
        ...
        float delta = 0f;
        if ( tweener.m_PreTime > 0 )
            delta = ( float )EditorApplication.timeSinceStartup - tweener.m_PreTime;
